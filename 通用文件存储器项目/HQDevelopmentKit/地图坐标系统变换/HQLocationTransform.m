...
@implementation HQLocationTransform
...
- (id)transformFromGPSToGD {
    CLLocationCoordinate2D coor = [HQLocationTransform transformFromWGSToGCJ:CLLocationCoordinate2DMake(self.latitude, self.longitude)];
    return [[HQLocationTransform alloc] initWithLatitude:coor.latitude andLongitude:coor.longitude];
}

- (id)transformFromGDToBD {
    CLLocationCoordinate2D coor = [HQLocationTransform transformFromGCJToBaidu:CLLocationCoordinate2DMake(self.latitude, self.longitude)];
    return [[HQLocationTransform alloc] initWithLatitude:coor.latitude andLongitude:coor.longitude];
}

- (id)transformFromBDToGD {
    CLLocationCoordinate2D coor = [HQLocationTransform transformFromBaiduToGCJ:CLLocationCoordinate2DMake(self.latitude, self.longitude)];
    return [[HQLocationTransform alloc] initWithLatitude:coor.latitude andLongitude:coor.longitude];
}

- (id)transformFromGDToGPS {
    CLLocationCoordinate2D coor = [HQLocationTransform transformFromGCJToWGS:CLLocationCoordinate2DMake(self.latitude, self.longitude)];
    return [[HQLocationTransform alloc] initWithLatitude:coor.latitude andLongitude:coor.longitude];
}

- (id)transformFromBDToGPS {
    //先把百度转化为高德
    CLLocationCoordinate2D start_coor = [HQLocationTransform transformFromBaiduToGCJ:CLLocationCoordinate2DMake(self.latitude, self.longitude)];
    CLLocationCoordinate2D end_coor = [HQLocationTransform transformFromGCJToWGS:CLLocationCoordinate2DMake(start_coor.latitude, start_coor.longitude)];
    return [[HQLocationTransform alloc] initWithLatitude:end_coor.latitude andLongitude:end_coor.longitude];
}
